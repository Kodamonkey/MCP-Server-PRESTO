...
def build_fold_qc_plan(pfd_file: str, goal: str | None = None) -> str:
    ...
    return (
        ...
        f"3. For interval/channel zapping, call `presto.pfdzap` with strict\n"
        f"   `<low>:<high>` tokens (experimental).\n"
        f"4. If you have a Gaussian template under DATA_DIR, call\n"
        f"   `presto.get_toas` for TOA generation.\n"
        f"5. To combine multiple folds, call `presto.sum_profiles`\n"
        f"   (experimental).\n\n"
...
def build_periodic_advanced_search_plan(
    ...
    return (
        ...
        f"Advanced add-ons:\n"
        f"- If a specific candidate is known (period or freq + DM),\n"
        f"  `presto.fourier_fold` produces a quick profile from the `.fft`\n"
        f"  (experimental).\n"
...
def build_tool_selection_guide(task: str) -> str:
    return (
        ...
        f"  `presto.pfd2png` (experimental), `presto.pfdzap` (experimental),\n"
        f"  `presto.fourier_fold` (experimental), `presto.sum_profiles`\n"
        f"  (experimental).\n"
